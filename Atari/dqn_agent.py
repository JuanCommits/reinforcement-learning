...
class DQNAgent(Agent):
    ...
    def update_weights(self, total_steps):
        ...
        if len(self.memory) > self.batch_size:
            ...
            next_values = rewards + (
                self.gamma * preds.max(1)[0].detach() * (1 - dones)
            )
            ...
            if (
                self.second_model_update is not None
                and total_steps % self.second_model_update == 0
            ):
