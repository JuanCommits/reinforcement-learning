...
class Agent:
    ...
    def compute_epsilon(self):
        ...
        return self.EPS_END + (self.EPS_START - self.EPS_END) * np.exp(-1. * self.steps_done * (1-self.EPS_DECAY))
    ...
    def optimize_model(self, optimizer):
        ...
        non_final_next_states = torch.cat([s for s in batch.next_state
                                                    if s is not None])
        next_states = torch.cat(batch.next_state_wo_none)
        state_batch = torch.cat(batch.state)
        action_batch = torch.cat(batch.action)
        reward_batch = torch.cat(batch.reward)
        dones_batch = torch.tensor(batch.done, device=self.device, dtype=torch.float16)

        #print("---------------------------------------------------------")

        # print(f"States {state_batch} - {state_batch.shape}")
        # print(f"Actions {action_batch} - {action_batch.shape}")
        # print(f"Rewards {reward_batch} - {reward_batch.shape}")
        # print(f"Next states {non_final_next_states} - {non_final_next_states.unsqueeze(1).shape}")
        ...
        #print(f"State_action_values {state_action_values} - {state_action_values.shape}")
        ...
        with torch.no_grad():
            next_state_values = self.target_net(next_states).max(1)[0].detach() * (1-dones_batch)
        # Compute the expected Q values
        expected_state_action_values = (next_state_values * self.GAMMA) + reward_batch

        #print(f"Expected_state_action_values values {expected_state_action_values} - {expected_state_action_values.shape}")
        ...
        if self.steps_done % self.second_model_update == 0:
                self.target_net.load_state_dict(self.policy_net.state_dict())
        ...
        # Initialize the environment and get its state
            ...
            for t in count():
                ...
                self.memory.push(state, action, next_state, reward, 
                                 ...
                                 torch.tensor([done], device=self.device, dtype=torch.float16))
